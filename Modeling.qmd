...
```{r}
(treeFit1 <- train(DiabetesStatus ~ BMI*GenHlth,
                  data = diab2,
                  method = "rpart",
                  trControl=trctrl,
...
> For Logistic Regression, the lowest logLoss function result was the formula analyzing Age, BMI, and their interaction effects.
...
```{r}
(glmpred <- predict(glmFit2, diabTest, type = "prob"))
```

```{r, warning=FALSE}
(logLoss(diabetes$Diabetes_binary, glmpred$Yes))
```

#### Classification Tree Prediction

```{r}
(ctpred <- predict(treeFit1, diabTest, type = "prob"))
```

```{r, warning=FALSE}
(logLoss(diabetes$Diabetes_binary, ctpred$Yes))
```

#### Random Forest Prediction

```{r}
(rfpred <- predict(treeFit2, diabTest, type = "prob"))
```

```{r, warning=FALSE}
logLoss(diabetes$Diabetes_binary, rfpred$Yes)
```
...
|----------------------:|:-----------:|
|     GLM (Age*BMI)     |   0.4492    |
|  Classification Tree  |   0.4422    |
|     Random Forest     |   0.4833    |
